...
function DataListPage() {
  ...
  useEffect(() => {
    fetchData();
    getProfile(); // Obtiene el perfil del usuario logueado
  }, []);
        ...
        {data.map((item, index) => (
          <tr key={index}>
            ...
            <td scope="col">
                ...
                {/* Botón de eliminar */}
                <button className={`btn ${item.id === user.id ? 'btn-secondary' : 'btn-danger'}`} disabled={item.id === user.id} onClick={() => handleDELETE(item.id)}>
